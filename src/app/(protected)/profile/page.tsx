  ...
  const handleEventToggle = (eventName: string) => {
    setProfileData(prev => {
      ...
      if (isSelected) {
        return {
          ...prev,
          eventsAttended: prev.eventsAttended.filter(event => event !== eventName)
        };
      } else {
        return {
          ...prev,
        ...
        <div className="px-4 py-6 space-y-6">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                {isEditing ? (
                  ...
                  <p className="text-gray-900">
                    {profileData.age ? `${profileData.age} years old` : 'Not specified'}
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                {isEditing ? (
                  ...
                  <p className="text-gray-900">{profileData.company || 'Not specified'}</p>
                )}
              </div>
            </div>
          </div>
            ...
            {isEditing ? (
              <div className="space-y-3">
                ...
                <div className="space-y-2 max-h-48 overflow-y-auto">
                  {availableEvents.map((event) => (
                    <label key={event.id} className="flex items-center gap-3 p-2 hover:bg-gray-50 rounded-md cursor-pointer">
                      <input
                        type="checkbox"
                        checked={profileData.eventsAttended.includes(event.name)}
              ...
              <div className="space-y-2">
                {profileData.eventsAttended.length > 0 ? (
                  ...
                  <p className="text-gray-500 text-sm">No events selected yet. Click Edit to add some!</p>
                )}
              </div>
            )}
          </div>
          ...
          {saveStatus === 'saved' && (
            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
              <p className="text-green-600 text-sm text-center">✅ Profile saved successfully!</p>
            </div>
          )}
          
          {saveStatus === 'error' && (
            <div className="bg-red-50 border border-red-200 rounded-lg p-4">
              <p className="text-red-600 text-sm text-center">❌ Failed to save profile. Please try again.</p>
            </div>
          )}
